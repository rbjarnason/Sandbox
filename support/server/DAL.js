            ...
            }, function(err) //done collecting children
            {
                ...
                async.whilst(function() {
                    ...
                    getInstance(cbparent, function(actualparent) {
                        ...
                        if (actualparent) {
                            returndata.parents.push({
                                world: cbparent,
                                type: cloneType,
                                created: actualparent.created,
                                title: actualparent.title,
                                children: []
                            });
                                ...
                                getInstance(item3, function(realitem3) {
                                    if (realitem3) {
                                        ...
                                        if (realitem3.publishedFrom)
                                            childtype = 1;
                                        returndata.parents[returndata.parents.length - 1].children.push({
                                            world: item3,
                                            type: childtype,
                                            created: realitem3.created,
                                            title: realitem3.title
                                        })
                                    } else {
                                        returndata.parents[returndata.parents.length - 1].children.push({
                                            world: item3,
...
function restoreBackup(id, stateFileName, cb) {
    ...
    var oldPath = libpath.join(basePath, stateFileName),
        statePath = libpath.join(basePath, 'state'),
        tempPath = libpath.join(basePath, 'temp');
